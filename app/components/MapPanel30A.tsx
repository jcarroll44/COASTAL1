...
export default function MapPanel30A({
  ...
  const nearest = useMemo(() => {
    if (!coordsOk || accessList.length === 0) return null;
    ...
    return best;
  }, [coordsOk, property?.lat, property?.lng, accessList]);

  const chosenAccess: Access | null = forceAccess ?? nearest;

  const distanceMiles =
    coordsOk && chosenAccess
      ? Math.round(
          (haversineMiles(
            { lat: property!.lat!, lng: property!.lng! },
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
      if (coordsOk && chosenAccess) {
        const b = new mapboxgl.LngLatBounds();
        b.extend([property!.lng!, property!.lat!]);
  ...
  useEffect(() => {
    ...
    if (lastStyleRef.current !== styleUrl) {
      ...
      // Style didn't change → repaint immediately (fixes "only after toggle" issue)
      repaint();
    }
  }, [
    styleUrl,
